...
def parse_args():
    ...
    parser.add_argument('--model_config', default='/home/abdelrahman.elsayed/sarim_code/model_svdtuning_encoder_decoder_prompt.yml',
...
def main_train(data_config, model_config, pretrained_path, save_path, training_strategy='biastuning', device='cuda:0'):
    ...
    if data_config['data']['name']=='LITS':
        ...
        no_text_mode = False
        for x in ['train', 'val']:
            dataset_dict[x] = ArcadeDataset(data_config, shuffle_list=True, is_train=(x=='train'), apply_norm=use_norm, no_text_mode=no_text_mode)
            dataloader_dict[x] = torch.utils.data.DataLoader(dataset_dict[x], batch_size=model_config['training']['batch_size'], shuffle=True, num_workers=4)
        dataset_sizes = {x: len(dataset_dict[x]) for x in ['train', 'val']}
        label_dict = {name: i for i, name in enumerate(data_config['data']['label_names'])}
    ...
    for name, p in model.named_parameters():
        ...
        if model_config['prompt_encoder'] == "full":
        ...
        if model_config['sam_encoder'] == 'full':
            if 'sam_encoder' in name.lower():
                print("unlock encoder")
                p.requires_grad = True
