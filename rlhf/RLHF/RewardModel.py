...
class RewardModel(nn.Module):
    def __init__(self , save_dir):
        ...
        self.save_dir = save_dir # Directory to save edge images
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir)
        
    def tensor_to_numpy(self, tensor):
        # Convert tensor to numpy array and ensure proper format for OpenCV
        if tensor.is_cuda:
            tensor = tensor.cpu()
        array = tensor.numpy()
    ...
    def detect_edges(self, mask, save_name=None, is_predicted=False):
        ...
        if len(mask.shape) == 2:  # Single mask
            ...
            # Save the image if save_name is provided
            if save_name:
    ...
    def forward(self, predicted_masks, ground_truth_masks, save_name=None):
        ...
        # Detect edges and save images if save_name is provided
