...
class Neck(nn.Module):
    ...
    def __init__(self, embed_dim, out_chans, mlp_transform=False, use_lora=False):
        ...
        if use_lora:
            ...
            rank_value = 250
            # self.conv1 = SALTConv2d(embed_dim, out_chans, kernel_size=1, bias=False , rank=rank_value , lora_rank=64)
            # self.conv2 = SALTConv2d(out_chans, out_chans, kernel_size=3, padding=1, bias=False , rank=rank_value , lora_rank=64)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if use_lora:
            ...
            # rank_value = 700
            # self.qkv = SALTLinear(dim, dim * 3, bias=qkv_bias , lora_rank=64 , rank=rank_value)
            # self.proj = SALTLinear(dim, dim , lora_rank=64 , rank=rank_value)
