...
with open('/l/users/sarim.hashmi/for_the_little_interns/the_experiments/drive_dataset/svd_only/config_arcade.yml', 'r') as data_config_file:
    data_config = yaml.safe_load(data_config_file)

# Load configuration from model_svdtuning.yml
with open('/l/users/sarim.hashmi/for_the_little_interns/the_experiments/drive_dataset/svd_only/model_svdtuning.yml', 'r') as model_config_file:
...
def train_dl(model, datasets, dataset_sizes, criterion, optimizer, scheduler, sav_path='./checkpoints/temp.pth', num_epochs=25, bs=32, device='cuda:0', retain_graph=False, neg2pos_ratio=-1, save_dir="./validation_images", reg_multiplier=0.01):
    ...
    # Initialize wandb
    wandb.init(project="Baselines_exp",name = "drive_svd_base", config={
